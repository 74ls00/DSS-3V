...
:menu
cls
echo port=%port% mk=%mk% programmer=%prog% itime=%itime%
echo tdir=%tdir%
echo fw=%fw%
...
goto menu
:id
%tdir%\avrdude.exe -c %prog% -p %mk% -P %port% -i %itime% -v F
pause
cls
goto menu
:wflash
%tdir%\avrdude.exe -p %mk% -P %port% -c %prog% -i %itime% -U flash:w:%fw%:a
pause
cls
goto menu
:wfuses
...
goto menu
:rflash
%tdir%\avrdude.exe -p %mk% -P %port% -c %prog% -B 4800 -i %itime% -U flash:r:"read.hex":r
pause
cls
goto menu
:vflash
%tdir%\avrdude.exe -p %mk% -P %port% -c %prog% -B 4800 -i %itime% -U flash:v:"%fw%":a
pause
cls
goto menu
:cerase
